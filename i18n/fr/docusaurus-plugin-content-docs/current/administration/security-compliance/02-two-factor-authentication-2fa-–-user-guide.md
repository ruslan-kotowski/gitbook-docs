---
title: Authentification à deux facteurs (2FA) – guide de l'utilisateur
...
:::tip
Découvrez comment activer l’authentification à deux facteurs (2FA) pour votre organisation pour les [forfaits Enterprise](../../enterprise-administration/security-integrations/two-factor-authentication-2fa/01-enterprise-two-factor-authentication-2fa-–-admin-guide.md), et [tous les autres forfaits](01-two-factor-authentication-2fa.md).
:::

## Comment mettre en place l’authentification à deux facteurs (2FA) ?

**Nouveaux utilisateurs :** Lors de la [procédure d’inscription](https://miro.com/signup/) initiale avec l’adresse e-mail de votre entreprise, vous serez invité à activer la fonction 2FA.
...
1. Téléchargez une application d’authentification sur votre appareil mobile. Les applications d'authentification, telles que Google Authenticator, Microsoft Authenticator et Authy, génèrent un code à usage unique basé sur le temps (TOTP) pour des connexions sécurisées à Miro. Pour savoir quelle application d’authentification choisir, adressez-vous à votre admin d’entreprise ou à votre administrateur informatique.

2. Cliquez sur **J'ai une application d’authentification** dans l'écran de configuration 2FA de Miro.
...
3. En utilisant l’application d’authentification, vous avez maintenant deux options :
   ...
   3. Après la numérisation, cliquez sur **J’ai numérisé le code**
      ...
      ![2FA-setup-step-2-Scan-QR-code.png](../../../../../../docs/administration/security-compliance/images/27601422721298_2FA-setup-step-2-Scan-QR-code.png)*Numérisation du code QR*

   Saisir manuellement le code Miro

   1. Si vous ne parvenez pas à scanner le code QR, cliquez sur **Impossible de scanner le code QR ?** dans Miro.
   ...
   4. Après avoir ajouté le code à l’application, cliquez sur **J’ai ajouté le code** dans Miro.
      ...
      ![2FA-setup-step-2-copy-code-and-add-to-authenticator-app..png](../../../../../../docs/administration/security-compliance/images/27601397098898_2FA-setup-step-2-copy-code-and-add-to-authenticator-app..png)*Copier le code Miro pour le coller dans l’application d’authentification*
...
5. Après avoir vérifié votre compte avec le code à 6 chiffres, Miro vous fournira un code de récupération. Cliquez sur **Copier** pour enregistrer ce code en toute sécurité. Il est crucial d’avoir ce code car il vous permet de réinitialiser votre 2FA au cas où vous perdriez l’accès à votre application d’authentification.

   Pour confirmer que vous avez bien enregistré le code, sélectionnez **J'ai enregistré ce code**, puis cliquez sur **Continuer** pour terminer le processus.

   ![Save-2FA-recovery-code.png](../../../../../../docs/administration/security-compliance/images/27601422727314_Save-2FA-recovery-code.png)*Sauvegarder le code de récupération*

## Connexion avec l'authentification à deux facteurs (2FA)

Une fois que vous avez configuré avec succès l’authentification à 2 facteurs (2FA) pour votre compte, chaque fois que vous tenterez de vous connecter, Miro vous demandera de saisir un code unique à 6 chiffres basé sur le temps (TOTP).

Ce code est généré par votre application d’authentification et ajoute un niveau de sécurité supplémentaire à votre compte. Il vous suffit d’ouvrir votre application d’authentification, de récupérer le code actuel et de le saisir sur la page de connexion pour accéder à votre compte.
...
Vous disposez de trois tentatives avant d’être invité à redémarrer le processus de connexion.

![Too-many-attempts.png](../../../../../../docs/administration/security-compliance/images/27601397107474_Too-many-attempts.png)*Trop de tentatives de connexion avec l’authentification à deux facteurs*
...
*La durée de confiance d’un appareil s’affiche à côté de la case à cocher lors de la connexion avec l’authentification à deux facteurs*

Si vous ne voyez pas l’option "Faire confiance à cet appareil", c’est que votre administrateur ne l’a pas activée pour votre organisation.

Si vous vous connectez avec un nouvel appareil, ou après avoir effacé les cookies de votre appareil de confiance, l’authentification à 2 facteurs sera à nouveau requise.

## Comment réinitialiser l’authentification à 2 facteurs (2FA)
...
2. Utilisez le code de récupération enregistré lors de la configuration initiale de la 2FA. Vous serez à nouveau guidé à travers le processus de configuration pour reconfigurer votre application d’authentification.

![Reset-two-factor-authentication.png](../../../../../../docs/administration/security-compliance/images/27601422733714_Reset-two-factor-authentication.png)*L’option de réinitialisation de l’authentification à deux facteurs*

### Je n’ai pas de code de récupération

Si vous avez perdu votre code de récupération ou ne pouvez pas utiliser le flux d’auto-récupération, vous devrez demander à votre admin de réinitialiser votre 2FA.

Les admins peuvent réinitialiser l’authentification à deux facteurs uniquement pour les utilisateurs dont les domaines d’e-mail sont vérifiés dans leur organisation, si l'admin initie la réinitialisation. Si l'utilisateur demande une réinitialisation, n'importe quel admin de l'organisation peut l'approuver.

1. Cliquez sur **Demander à votre admin de réinitialiser**.
   ![2fa-user-reset.png](../../../../../../docs/administration/security-compliance/images/27601397113106_2fa-user-reset.png)
   *Demandez à votre admin de réinitialiser votre authentification à 2 facteurs si vous n'avez pas de code de récupération*
2. Si vous appartenez à plus d'une organisation utilisant l’authentification à 2 facteurs, vous devrez choisir l’admin de quelle organisation vous souhaitez envoyer la demande.
3. Vous recevrez un e-mail avec un code de vérification.
4. Saisissez le code de vérification.
5. Une confirmation que la demande a été envoyée à l'admin que vous avez choisi sera affichée.
6. Lorsque l'admin réinitialise votre 2FA, vous devrez passer par le processus de configuration 2FA lors de votre prochaine connexion.

## Foire aux questions

Pourquoi dois-je configurer 2FA ?

L’authentification à deux facteurs renforce la sécurité de votre organisation.
Tous les utilisateurs non-SSO doivent utiliser l’authentification
à deux facteurs pour se connecter si cette exigence est appliquée par
votre admin d’entreprise.
