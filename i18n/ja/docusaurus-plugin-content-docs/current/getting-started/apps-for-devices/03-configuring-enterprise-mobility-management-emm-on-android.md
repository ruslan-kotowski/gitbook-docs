---
title: Android でのエンタープライズ モビリティ管理（EMM）の設定
...
availability:
  notes: '一般的な: 手順を説明します：VMWare（WorkspaceONE）、Ivanti Neurons（旧MobileIron Cloud）、Intune（Microsoft
...
多くの場合、EMM 設定を有効にするには、Miro を組織のアプリケーション カタログに追加する必要があります。このプロセスは、EMM プロバイダーごとに異なることもあります。通常は、Google Play ストアから Miro を直接アプリケーション カタログに追加し、デバイスグループ、ユーザーグループなどに基づいて配布ポリシーを設定することになります。
...
全般的な Ivanti Neurons 導入ガイドは、[こちら](https://help.ivanti.com/mi/help/en_us/cld/admin/ivanti/92/all/Managing_Google_Play_apps.htm)からご覧いただけます。

- 1. **[アプリ] > [アプリカタログ]** に移動し、**[追加]** をクリックします。
  2. **[Google Play ストア]** を選択し、ソースとしてお住まいの国を選択します。
  3. 検索 "**Miro**"を検索し、"**Miro "を選択してください：オンラインホワイトボード**" 利用可能なアプリケーションのリストから。
  4. 組織の要件に応じて、割り当てと配布設定を構築します。

**Intune（Microsoft Endpoint Manager）**

MS による Intune の全般的な導入ガイドは[こちら](https://learn.microsoft.com/mem/intune/fundamentals/deployment-guide-platform-android)からご覧いただけます。

1. **[アプリ] > [すべてのアプリ]** を開き、**[追加]**をクリックします。
2. **[ストア アプリ] > [Android ストア アプリ]** を**アプリの種類**として選択します。
3. **「アプリ情報」**ページで、Google Play ストアにある [Miro の詳細を情報入力します](https://play.google.com/store/apps/details?id=com.realtimeboard)。
4. 組織の要件に応じて、割り当てと配布設定を構築します。
...
Miro は、[AppConfig](https://www.appconfig.org/) を顧客のデータを設定し保護するための統一された方法として使用しており、エンタープライズ モバイル アプリケーションを簡単に設定できます。多くの EMM ソリューションは AppConfig 形式に対応しています。もしくは、互換モードで対応しています。　お客様のケースに該当する細かい制約については、ご利用の EMM プロバイダーのドキュメンテーションをご覧ください。
...
全般的な Ivanti Neurons 導入ガイドは、[こちら](https://help.ivanti.com/mi/help/en_us/cld/admin/ivanti/92/all/Using_the_Android_enterprise_App_Configuration.htm)からご覧いただけます。

1. **[アプリ] > [アプリカタログ]** に移動します。
2. **Miroに移動します：オンラインホワイトボード**" セッティング.
3. **[アプリ構成] > [Android用マネージド構成]**に移動します。
4. **[追加]** をクリックして、**アプリの制約**を作成します。
5. **[Required]**（必須）と **[Runtime** **Permissions]**（ランタイム権限）を定義します。
6. **「このアプリ構成の配布」**セクションで、配布プロファイルを選択します。

**Intune（Microsoft Endpoint Manager）**

MS による Intune の全般的な導入ガイドは[こちら](https://learn.microsoft.com/mem/intune/apps/app-configuration-policies-use-android)からご覧いただけます。

1. **[アプリ] > [アプリの構成ポリシー] > [追加] > [マネージドデバイス]** に移動し、新しいアプリケーション構成を作成します。
2. 構成プロファイル名を定義します。
3. **プラットフォーム**として **Android Enterprise** を選択します。
4. **Miroを選択して ください：オンラインホワイトボード** 。 **ターゲットアプリ** をクリックします。 **アプリを選択**.
5. **構成設定フォーマット**として **[構成デザイナーを使用する]** を選択します。
6. アプリケーション構成を定義します。
7. 構成の配布プロファイルを選択します。
