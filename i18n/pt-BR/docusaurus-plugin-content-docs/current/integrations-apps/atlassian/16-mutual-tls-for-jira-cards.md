---
title: "Mutual TLS para cart\xF5es do Jira"
article_id: 4410562720658
translation_id: 14537640278034
locale: pt-br
sidebar_position: 15
created_at: '2023-10-19T15:38:40Z'
updated_at: '2026-03-27T16:18:33Z'
draft: false
outdated: false
...
A Mutual Transport Layer Security permite estabelecer uma conexão ainda mais segura entre sua instância Jira e a Miro. A funcionalidade é automaticamente suportada em todos os planos Enterprise e *não requer nenhuma configuração no lado da Miro.*

:::warning
Note que este artigo não apresenta instruções detalhadas, mas apenas fornece *uma configuração de exemplo e nosso certificado* (no final do artigo). Por favor, consulte sua equipe de TI e seus administradores de sistema, pois dependendo da sua infraestrutura de rede, as etapas de configuração podem variar.
:::

## Formas de configurar

Existem duas opções para escolher em relação a como a validação será verificada:

- Via o certificado
- Via a impressão digital do certificado

Escolha o método que preferir e ajuste a configuração do NGINX que você possui usando um dos seguintes trechos de código. Certifique-se de substituir [127.0.0.1](http://127.0.0.1/jira/plugins/servlet/oauth/authorize) pelo IP ou endereço web da sua instância do Jira e insira nossos valores de certificado em vez de ENTER_MIRO_CERTIFICATE_HERE.

### Validação via certificado

Encontre o exemplo para a configuração do NGINX abaixo:

```
ssl_verify_client optional; ssl_verify_depth 3; set $cert_old "INSIRA_CERTIFICADO_ANTIGO_DA_MIRO_AQUI"; set $cert_new "INSIRA_CERTIFICADO_NOVO_DA_MIRO_AQUI"; set $valid_cert_flag 0; location /jira/plugins/servlet/oauth/authorize { proxy_pass http://127.0.0.1/jira/plugins/servlet/oauth/authorize; proxy_set_header X-Forwarded-Host $host; proxy_set_header X-Forwarded-Server $host; proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for; client_max_body_size 10M; proxy_redirect off; } location /jira/login.jsp { proxy_pass http://127.0.0.1/jira/login.jsp; proxy_set_header X-Forwarded-Host $host; proxy_set_header X-Forwarded-Server $host; proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for; client_max_body_size 10M; proxy_redirect off; } location /jira { if ($ssl_client_raw_cert ~ $cert_old) { set $valid_cert_flag 1; } if ($ssl_client_raw_cert ~ $cert_new) { set $valid_cert_flag 1; } if ($valid_cert_flag != 1) { return 403 "Certificado inválido\n"; } proxy_pass http://127.0.0.1/jira; proxy_set_header X-Forwarded-Host $host; proxy_set_header X-Forwarded-Server $host; proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for; client_max_body_size 10M; proxy_redirect off; }
```

### Validando através da impressão digital do certificado

Encontre abaixo o exemplo de configuração para o NGINX:

```
ssl_verify_client optional;
ssl_verify_depth 3;
set $fingerprint_old "INSIRA_AQUI_O_OLD_FINGERPRINT_DO_CERTIFICADO_MIRO";
set $fingerprint_new "INSIRA_AQUI_O_NEW_FINGERPRINT_DO_CERTIFICADO_MIRO";
...
location /jira {
  if ($ssl_client_fingerprint = $fingerprint_old) {
    set $valid_fingerprint_flag 1;
  }
  if ($ssl_client_fingerprint = $fingerprint_new) {
    set $valid_fingerprint_flag 1;
  }
  if ($valid_fingerprint_flag != 1) {
    return 403;
  }
...
Novo certificado Certificado antigo

O novo certificado é válido de 14 de abril de 2026 até 9 de outubro de 2026.
...
O certificado antigo é válido até 14 de abril de 2026.
